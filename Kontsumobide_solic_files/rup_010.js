		...
		_init : function(args) {
			if (args.length > 1) {
				...
				settings.gridComplete = function(){
					...
					$("#"+$(this).attr("id")+" [title]").rup_tooltip({show:{delay:settings.tooltipDelay}});
				};
				...
				settings.onPaging = function(pgButton){
					...
					if (pgButton === 'user'){
						// TODO BILBOMATICA Cambio para que funcione la paginacion con más de 999 registros (problema del .)
						var page = parseInt($("#gbox_"+$(this).attr("id")).find(".pagControls > input").val()),
							lastPage = parseInt($("#gbox_"+$(this).attr("id")).find(".pagControls > input").next().text().replace(".", ""));
					...
					,gridComplete_default: function(){
						...
						if (self.jqGrid("getDataIDs").length === 0) {//es que no hay registros
							...
							if ($("#RUP_GRID_" + self[0].id + "_noRecords").length === 0) {
								...
								content += '<td aria-describedby="RUP_GRID_' + self[0].id + '_NO_REGISTROS" title="' + $.rup.i18nParse($.rup.i18n.base,"rup_grid.noRecordsFound") + '" style="padding-left: 0.5em ! important;text-align: left;width:' + $("#gview_"+$(self).attr("id")).width() + 'px;background:white;" role="gridcell">';
...
//							}
				...
				if(settings.fluidBaseLayer===null){
					settings.fluidBaseLayer = "#gbox_"+$(self).attr("id");
				}
